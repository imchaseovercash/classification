...
def enhancedFeatureExtractorDigit(datum):
    ...
    reward prev pixels if they bigger, reward curr pixels if they bigger
    lower prev pixels if they smaller, lower curr pixels if they smaller
    ##
    """
    ...
    for w in width:
        for h in height:
            prev_pixel = datum.getPixel(w, h-1)
            curr_pixel = datum.getPixel(w, h)
...
def enhancedPacmanFeatures(state, action):
    ...
    successor = state.generateSuccessor(0, action)
    """"""
    dist_to_food = [util.manhattanDistance(successor.getPacmanPosition(), food) for food in state.getFood().asList()]
    food_length = len(dist_to_food)
    if food_length > 0:
        features['foodMin'] = min(dist_to_food)
    """"""
    if successor.getPacmanPosition() != state.getPacmanPosition():
        features['not_stop'] = 1
    """"""
    if successor.getFood().count() - state.getFood().count() > 0:
        features['food_increase'] = 1
    """"""
    features['score_increase'] = successor.getScore() - state.getScore()
    """"""
    return features
